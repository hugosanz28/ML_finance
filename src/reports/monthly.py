...
def generate_monthly_report(
    *,
    ...
    resolved_snapshots = _prepare_snapshots_frame(
        load_normalized_degiro_snapshots(
            settings=resolved_settings,
            normalized_degiro_dir=normalized_degiro_dir,
        )
    ...
    report_notes = _build_report_notes(
        ...
        period_summaries=period_summaries,
        cash_movements=resolved_cash_movements,
        broker_snapshot_date=(broker_snapshot["snapshot_date"] if broker_snapshot is not None else None),
    )
...
def _prepare_snapshots_frame(frame: pd.DataFrame) -> pd.DataFrame:
    if frame is None or frame.empty:
        return pd.DataFrame(
            columns=[
                "snapshot_date",
    ...
    ready = frame.copy()
    if "snapshot_date" not in ready.columns or "asset_id" not in ready.columns:
        return pd.DataFrame(
            columns=[
                "snapshot_date",
                "asset_id",
    ...
    for column in ("asset_name", "asset_type", "isin", "quantity", "market_value_base", "unrealized_pnl_base"):
        if column not in ready.columns:
            ready[column] = pd.NA
    ready["snapshot_date"] = pd.to_datetime(ready["snapshot_date"], errors="coerce").dt.date
    ready["asset_name"] = ready["asset_name"].fillna(ready["asset_id"]).astype("string")
    ready["asset_type"] = ready["asset_type"].fillna("unknown").astype("string")
    for column in ("quantity", "market_value_base", "unrealized_pnl_base"):
        ready[column] = pd.to_numeric(ready[column], errors="coerce")
    return ready.dropna(subset=["snapshot_date", "asset_id", "market_value_base"]).reset_index(drop=True)
...
def _apply_broker_snapshot_override(
    *,
    daily_as_of: pd.Series,
    ...
    if total_unrealized is not None:
        updated_daily["total_unrealized_pnl_base"] = float(total_unrealized)
    if total_return is not None:
        updated_daily["portfolio_return_pct"] = float(total_return)

    updated_periods: list[MonthlyPeriodSummary] = []
    for summary in period_summaries:
        value_start = float(summary.total_market_value_start_base)
        value_change = round(total_market_value - value_start, 8)
        value_change_pct = None
        if abs(value_start) > 1e-12:
            value_change_pct = round(total_market_value / value_start - 1, 8)
        updated_periods.append(
            replace(
                summary,
                total_market_value_end_base=round(total_market_value, 8),
                total_market_value_change_base=value_change,
                total_market_value_change_pct=value_change_pct,
                portfolio_return_pct_end=None if total_return is None else round(float(total_return), 8),
            )
        )
...
def _build_report_notes(
    *,
    ...
    period_summaries: tuple[MonthlyPeriodSummary, ...],
    cash_movements: pd.DataFrame,
    broker_snapshot_date: date | None = None,
) -> tuple[str, ...]:
    notes: list[str] = [
        (
            "El valor total y la asignacion actual usan snapshot DEGIRO cuando existe para la fecha de referencia."
            if broker_snapshot_date is not None
            else "Las valoraciones usan el ultimo precio disponible en o antes de cada fecha, segun la logica actual de metrics."
        ),
    ...
    missing_positions = positions_as_of.loc[
        ~positions_as_of["valuation_status"].isin(["valued", "valued_cash", "broker_snapshot"]),
        "asset_name",
    ].astype("string")
